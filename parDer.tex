...
We follow the same strategy as in \cref{sec:height} and define in \cref{fig:incFunSize} the function $\incFunSize{\re}$, analogous to $\incFun{\re}$, but  for the size of $\re$.
...
The definition of $\incFunSize{\re}$ is driven by the reduction rules in \cref{fig:parDer}. We comment only the non-trivial cases: if $\re=\re_0\orop\re_1$, then we expect $\tsize{\re'_i}\leq\tsize{\re_i}+1$, $i=0,1$,
hence $\tsize{\re'_i}\leq\tsize{\re}$ and $\incFunSize{\re}=0$;
if $\re=\starop{\re_0}$, then we expect $\tsize{\re'_0}\leq\tsize{\re_0}+1$,
hence $\tsize{\re'_0\catop\starop{\re_0}}\leq\tsize{\re}+1$ and $\incFunSize{\re}=1$. Finally, the definition for $\re=\re_0\catop\re_1$ requires more care:
if rule $\rn{l-cat}$ is applied, then $\tsize{\re_0'\catop\re_1}=\tsize{\re}+1$, but only when $\tsize{\re_0'}=\tsize{\re_0}+1$ and $\tsize{\re_0}\geq\tsize{\re_1}$, otherwise $\tsize{\re_0'\catop\re_1}\leq\tsize{\re}$; if rule $\rn{r-cat}$ is applied, then $\tsize{\re'_1}\leq\tsize{\re}$, as happens for rule $\rn{r-or}$. Therefore $\incFunSize{\re}=1$ iff $\incFunSize{\re_0}=1$ and $\tsize{\re_0}\geq\tsize{\re_1}$, that is, $\geqFun{\re_0}{\re_1}=1$. In all other cases, that is $\incFunSize{\re_0}=0$ or $\geqFun{\re_0}{\re_1}=0$, $\incFunSize{\re}=0$.

The following theorem proves the soundness of the definition of $\incFunSize{}$ \wrt~ its intended meaning. As a consequence, the height of the derivative of an expression $\re$ \wrt~ a symbol is always bounded by $\tsize{\re}+1$.
\begin{theorem}\label[theorem]{theo:inc-bound}
 For all $\re,\re'\in\reSet$, $\sym\in\symAlph$, if $\re\parDer{\sym}\re'$, then $\tsize{\re'}\leq \tsize{\re}+\incFunSize{\re}$.
\end{theorem}
\begin{proof}
 By induction and case analysis on the rules defining $\re\parDer{\sym}\re'$.
 \begin{description}
  \item[base case:] The only base rule is \rn{sym}.
   Hence, $\re=\sym$, $\re'=\eps$, and
   $\tsize{\eps}=0=\tsize{\sym}=\tsize{\sym}+0=\tsize{\sym}+\incFunSize{\sym}$.
  \item[inductive step:] the proof proceeds by case analysis on the rule applied
   at the root of the derivation tree.
   \begin{itemize}
    \item $\Rule{l-cat}{\re_0\parDer{\sym}\re'_0}{\re_0\catop\re_1\parDer{\sym}\re'_0\catop\re_1}{}$\\[2ex]
          ...
          We distinguish two cases:
          \begin{itemize}
           \item $\tsize{\re_0}\geq\tsize{\re_1}$:
                 $\tsize{\re_0'\catop\re_1}\stackrel{\mathrm{def}}{=}\max(\tsize{\re'_0},\tsize{\re_1})+1\leq\max(\tsize{\re_0}+\incFunSize{\re_0},\tsize{\re_1})+1\leq\max(\tsize{\re_0},\tsize{\re_1})+1+\incFunSize{\re_0}=\tsize{\re_0\catop\re_1}+\incFunSize{\re_0}=\tsize{\re_0\catop\re_1}+\incFunSize{\re_0\catop\re_1}$, by the inductive hypothesis, the definition of $\max$, $\tsize{\ }$, $\incFunSize{}$, and $\geqSym()$, the assumption $\tsize{\re_0}\geq\tsize{\re_1}$, and \cref{lemma:bounds}.

           \item $\tsize{\re_0}<\tsize{\re_1}$:
                 $\tsize{\re_0'\catop\re_1}\stackrel{\mathrm{def}}{=}\max(\tsize{\re'_0},\tsize{\re_1})+1\leq\max(\tsize{\re_0}+\incFunSize{\re_0},\tsize{\re_1})+1=\tsize{\re_0\catop\re_1}=\tsize{\re_0\catop\re_1}+\incFunSize{\re_0\catop\re_1}$, by the inductive hypothesis, the definition of $\max$, $\tsize{\ }$, $\incFunSize{}$, and $\geqSym()$, the assumption $\tsize{\re_0}<\tsize{\re_1}$, and \cref{lemma:bounds}.
          \end{itemize}
...
The following corollary can be directly derived from \cref{theo:inc-bound} and \cref{lemma:bounds}.
\begin{corollary}\label[corollary]{cor:bound}
 For all $\re,\re'\in\reSet$, and $\sym\in\symAlph$, if $\re\parDer{\sym}\re'$, then $\tsize{\re'}\leq\tsize{\re}+1$.
\end{corollary}
~\Cref{theo:inc-bound} shows that the height of the derivative of an expression $\re$ \wrt~ a symbol is bounded by $\tsize{\re}+1$.
...
\begin{lemma}\label[lemma]{lemma:zero-inc}
 For all $\re,\re'\in\reSet$, and $\sym\in\symAlph$, if $\re\parDer{\sym}\re'$, then $\incFunSize{\re'}=0$.
\end{lemma}
\begin{proof}
 By induction and case analysis on the rules defining $\re\parDer{\sym}\re'$.
 \begin{description}
  ...
  \item[inductive step:] the proof proceeds by case analysis on the rule applied
   ...
   \begin{itemize}
    \item $\Rule{l-cat}{\re_0\parDer{\sym}\re'_0}{\re_0\catop\re_1\parDer{\sym}\re'_0\catop\re_1}{}$\\[2ex]
          If $\geqFun{\re_0}{\re_1}=1$, then by definition of $\incFunSize{}$ and inductive hypothesis, $\incFunSize{\re'_0\catop\re_1}=\incFunSize{\re'_0}=0$.
          If $\geqFun{\re_0}{\re_1}=0$, then by definition of $\incFunSize{}$ $\incFunSize{\re'_0\catop\re_1}=0$.

    \item $\Rule{r-cat}{\re_1\parDer{\sym}\re'_1}{\re_0\catop\re_1\parDer{\sym}\re'_1}{\hasEps{\re_0}=\eps}$\\[2ex]
          $\incFunSize{\re'_1}=0$ directly follows from the inductive hypothesis.
...
We can now generalize \Cref{theo:inc-bound} to derivatives \wrt~ any words.
\begin{theorem}\label[theorem]{theo:gen-inc-bound}
 For all $\re,\re'\in\reSet$, and $\word\in\symAlph^*$, if $\re\parDer{\word}\re'$, then $\tsize{\re'}\leq\tsize{\re}+\incFunSize{\re}$.
\end{theorem}
\begin{proof}
 By induction on the length of $\word$.
 \begin{description}
...
\Cref{lemma:bounds} allows to conclude the final result stating that the height of the derivatives of $\re$ is bounded by $\tsize{\re}+1$.
\begin{corollary}\label[corollary]{cor:gen-inc-bound}
 For all $\re,\re'\in\reSet$, and $\word\in\symAlph^*$, if $\re\parDer{\word}\re'$, then $\tsize{\re'}\leq\tsize{\re}+1$.
\end{corollary}