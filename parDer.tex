...
The following theorem states that the height of the derivative of an expression $\re$ w.r.t. a symbol is bounded by $\theight{\re}+1$.
\begin{theorem}\label[theorem]{inc-bound}
...
The following corollary can be directly derived from \cref{inc-bound} and \cref{bounds}.
\begin{corollary}
...
\begin{theorem}\label[theorem]{zero-inc}
...
We can now generalize \Cref{inc-bound} to derivatives w.r.t. any words.
\begin{theorem}\label[theorem]{gen-inc-bound}
...
\Cref{bounds} allows to conclude the final result stating that the height of the derivatives of $\re$ is bounded by $\theight{\re}+1$.
\begin{corollary}
